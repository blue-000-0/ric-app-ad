...
class DATABASE(object):
    ...
    def read_data(self, train=False, valid=False, limit=False):
        ...
        if not train and not valid and not limit:
            ...
            query += ' |> range(start: -1200m, stop: -1180m) '
            query += ' |> filter(fn: (r) => r["_measurement"] == "UeMetrics") '
            query += ' |> filter(fn: (r) => r["_field"] == "DRB_UEThpDl" or r["_field"] == "Viavi_UE_Rsrp" or r["_field"] == "Viavi_UE_Rsrq" or r["_field"] == "Viavi_UE_RsSinr" or r["_field"] == "RRU_PrbUsedDl" or r["_field"] == "Viavi_UE_anomalies") '
            query += ' |> pivot(rowKey: ["_time"], columnKey: ["_field"], valueColumn: "_value") '
            query += ' |> limit(n: 20)'
        elif valid:
            query += ' |> range(start: -1180m, stop: -1165m) '
            query += ' |> filter(fn: (r) => r["_measurement"] == "UeMetrics") '
            query += ' |> filter(fn: (r) => r["_field"] == "DRB_UEThpDl" or r["_field"] == "Viavi_UE_Rsrp" or r["_field"] == "Viavi_UE_Rsrq" or r["_field"] == "Viavi_UE_RsSinr" or r["_field"] == "RRU_PrbUsedDl" or r["_field"] == "Viavi_UE_anomalies") '
            query += ' |> pivot(rowKey: ["_time"], columnKey: ["_field"], valueColumn: "_value") '
            query += ' |> limit(n: 15)'
        ...
        self.data = result
        ## print(self.data.head(1).to_string())
    ...
    def write_anomaly(self, df, meas='AD'):
        ...
        try:
            ...
            write_api.write(bucket=self.bucket, record=df, data_frame_measurement_name=meas)
