...
class DATABASE(object):
    ...
    def connect(self):
        ...
        try:
            self.client = DataFrameClient(host=self.host, port=self.port, username=self.user, password=self.password, database=self.dbname, ssl=self.ssl, verify_ssl=self.ssl, path=self.path)
